...
// VALIDATION SCHEMA
const blogContactValidationSchema = joi.object({
  ...
  return `
    ...
    <html>
      <head>
        ...
        <title>New Blog Contact Submission</title>
        <style>
          body, html {
            margin: 0;
            padding: 0;
      ...
      <body>
        <div class="email-wrapper">
          ...
          <div class="content">
            <h2><span class="emoji">📩</span> New Blog Contact Form Submission</h2>
            <p class="highlight">You have received a new blog contact form submission with the following details:</p>
            <table>
              <tr>
                <th>Name</th>
                <td>${name}</td>
          ...
          <div class="footer">
            This email was automatically generated by your website's blog contact form.
  ...
  <body>
    <div class="email-wrapper">
      ...
      <div class="content">
        ...
        <p>If you have any urgent questions, please don't hesitate to call us directly at <strong>+91 1234567890</strong>.</p>
      </div>
      <div class="footer">
...
const handler = async (req, res) => {
  ...
  try {
    ...
    let { error } = blogContactValidationSchema.validate({ name, email });
    if (error) {
      return res
        .status(400)
        .json({ isSuccess: false, message: "Validation Error", error });
    }
    let isDataExist = await blogModel.findOne({
      $or: [{ email }],
    });
    if (isDataExist) {
      return res
    ...
    let newBlogContact = new blogModel(req.body);
    let isSaved = await newBlogContact.save();
    if (isSaved) {
      await Promise.all([
        await sendMail(
          SMTP_MAIL,
        ...
        await sendMail(
          SMTP_MAIL,
          SMTP_MAIL,
          `New Blog Contact Request from ${name}`,
          firmTemplate(req.body)
        ),
      ]);
      res.status(201).json({
        isSuccess: true,
        message: "New Blog Contact Details Added Succesfully",
      });
    } else {
      return res.status(400).json({
        isSuccess: false,
        message: "Error While Inserting Blog Contact Details",
      });
    }
  } catch (error) {
    return res
